...
async function appenddata() {
  try {
    let responsedata = await fetch(
      "https://express-clone-backend.onrender.com/mens/get"
    );
...
async function lowtohigh() {
  let responsedata = await fetch(
    "https://express-clone-backend.onrender.com/mens/getbyprice?price=1"
  );
...
async function filterfunc(a = 0, b = 200) {
  console.log(a, b);
  let responsedata = await fetch(
    `https://express-clone-backend.onrender.com/mens/filteredprice?price=${a}&price=${b}`
  );
...
async function cart(payload, index) {
  ...
  let responsedata = await fetch(
    "https://express-clone-backend.onrender.com/carts/addtocart",
    {
      method: "POST",
      headers: {
